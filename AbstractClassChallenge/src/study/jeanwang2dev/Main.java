...
//        MyNodeList myNodeList = new MyNodeList(null);
//        myNodeList.traverse(myNodeList.getHeadNode());
//
//        for (String s : data) {
//            // create new item with value set to the string s
//            myNodeList.add(new StrNode(s));
//        }
//
//        myNodeList.traverse(myNodeList.getHeadNode());
//        MyNode nodeToRemove = new StrNode("Darwin");
//        myNodeList.remove(nodeToRemove);
//        myNodeList.traverse(myNodeList.getHeadNode());
//
//        // try IntNode
//        int[] intArray = {12, 334, 10, 4, 45, 67, 1234, 890};
//        MyNodeList myNodeList2 = new MyNodeList(null);
//        myNodeList2.traverse(myNodeList2.getHeadNode());
//
//        for(int number : intArray) {
//            myNodeList2.add(new IntNode(number));
//        }
//        myNodeList2.traverse(myNodeList2.getHeadNode());
//        nodeToRemove = new IntNode( 45);
//        myNodeList2.remove((nodeToRemove));
//        myNodeList2.traverse(myNodeList.getHeadNode());

        // Binary Search Tree
        int[] intArray2 = {12, 443, 10, 4, 45, 67, 1234, 443, 890};
        BST myBst = new BST(null);
        for(int value : intArray2) {
            if( myBst.add(new IntNode(value) )){
                System.out.println("value added for " + value);
            } else {
                System.out.println("error");
            }
        }
        myBst.traverse(myBst.getHeadNode());
        myBst.remove(new IntNode(10));
        myBst.traverse(myBst.getHeadNode());

        myBst.remove(new IntNode(890));
        myBst.traverse(myBst.getHeadNode());

        String stringData = "Darwin Brisbane Perth Melbourne Canberra Adelaide Sydney Canberra";

        String[] data = stringData.split(" ");
        BST myBst2 = new BST(null);
        for (String s : data) {
            // create new item with value set to the string s
            myBst2.add(new StrNode(s));
        }
        myBst2.traverse(myBst2.getHeadNode());
//        MyNode node1 = new IntNode(443);
//        MyNode node2 = new IntNode( 443);
